...
def map_names_ids(token, org_name):
	...
	json_result = json.dumps(r.text)
	team_map = dict()
	for team_data in json_result: 
		if team_data['name'].contains('Group'):
			team_map[team_data['name']] = team_data['id']
	return team_map

def add_member(token, org_name, team_map, username, team_number):
	headers = {'Authorization': ' token ' + token, 'Content-type': 'application/json', 'Accept': 'application/vnd.github.v3+json'}
	team_id = team_map['/Group' + team_number]
	add_member_url = template_url + orgs_url + org_name + teams_url + team_id + memberships_url + username
...
def parse_csv(token, org_name, csv_file):
	...
	for row in reader: add_member(token, org_name, team_map, row.split(',')[0], 'Group' + row.split(',')[1])

# main method
def main():
	if len(sys.argv) < 4:
		print 'Usage: github-groupmaker.py org_name csv_file oauth_key'
		exit()
	org_name = sys.argv[1]
